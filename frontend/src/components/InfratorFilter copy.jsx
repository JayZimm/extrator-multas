...
function InfratorFilter() {
  ...
  useEffect(() => {
    if (isChanging) {
      // Atualiza o queryClient quando o infrator selecionado mudar
      queryClient.setQueryData(['infrator'], selectedInfrator);
      
      // Adicionar logging para depuração
      console.log('[InfratorFilter] Alterado infrator para:', selectedInfrator);
      
      // Primeiro, garantir que a mudança seja propagada para quem observa a query 'infrator'
      queryClient.invalidateQueries({
        queryKey: ['infrator'],
        exact: true
      });
      
      // Remover o cache de autos para garantir uma nova busca
      queryClient.removeQueries(['autos']);
      
      // Invalidar e forçar refetch da query 'autos' imediatamente
      queryClient.invalidateQueries({
        queryKey: ['autos'],
        exact: false,
        refetchActive: true
      ...
      console.log('[InfratorFilter] Cache de autos removido e query invalidada');
      
      // Reset do estado
      setIsChanging(false);
    }
  }, [selectedInfrator, queryClient, isChanging]);
